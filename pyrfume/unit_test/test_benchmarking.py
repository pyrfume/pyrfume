    ...
    def test_add_features(self):
        ...
        dataset.add_features(
            ...
            features=pd.DataFrame(
                {'f1': [6, 7, 8, 9, 10], 'f2': [11, 12, 13, 14, 15]}
            ).rename_axis('CID')
        )
...
class TestModel(unittest.TestCase):
    ...
    def test_instance(self):
        ...
        model.set_parameter_grid(
            param_grid={
                'minmaxscaler': {'clip': [True, False]}
            }
        )
...
class TestBenchmarking(unittest.TestCase):
    ...
    def test_resolve_scoring(self):
        for task in ['classification', 'regression']:
            self.assertTrue(all(
                scorer in SCORERS
                for scorer in pbm.resolve_scoring(scoring=None, task=task)
            ))
    ...
    def test_list_default_estimators(self):
        ...
        self.assertTrue(all(
            estimator in {
                **pbm.DEFAULT_PARAMETERS_CLASSIFIERS,
                **pbm.DEFAULT_PARAMETERS_REGRESSORS
            }
    ...
    def test_reformat_parameter_grid(self):
        model = pbm.Model(
            ...
            param_grid={'alpha': [1, 5, 10], 'fit_prior': [True, False]}
        )
        model.add_feature_selection(
            score_function='f-statistic',
            mode='k_best',
    ...
    def test_gridsearch_results_to_dataframe(self):
        ...
        self.assertTrue(all(
            col in df.columns
            for col in ['pipeline_steps', 'pipeline_string', 'param_string']
        ))
    ...
    def test_reconstruct_model(self):
        ...
        results = pd.DataFrame({
            ...
            'max_error_param_string': {'LinearRegression': 'linearregression__fit_intercept=True'},
